"""Stage 23: Creator Impersonation Detection (s23_impersonation_detection.py)
...
def run_impersonation_detection():
    ...
    stage_cfg = config.get("stage_23_impersonation", {})
    min_name_length = int(stage_cfg.get("min_name_length", 6))
    min_unique_accounts = int(stage_cfg.get("min_unique_accounts", 2))
    ...
    valid_names = df[df['name_len'] >= min_name_length].copy()
    ...
    suspects = impersonation[impersonation['unique_accounts'] >= min_unique_accounts].copy()
