"""Stage 26: Integrity & Spam Flags (s26_integrity.py)
...
def run_integrity_detection():
    ...
    stage_cfg = config.get("stage_26_integrity", {})
    lsh_threshold = float(stage_cfg.get("lsh_threshold", 0.8))
    lsh_num_perm = int(stage_cfg.get("lsh_num_perm", 128))
    burst_window_minutes = int(stage_cfg.get("burst_window_minutes", 5))
    burst_multiplier = float(stage_cfg.get("burst_multiplier", 10.0))
    ...
    # Using configured permutations and threshold Jaccard similarity
    ...
    bin_freq = f"{burst_window_minutes}min"
    for video_id, group in df.groupby('video_id'):
        # Resample to configured time bins and count comments
        time_series = group.set_index('published_at').resample(bin_freq).size()
        ...
        # Burst threshold: configured multiplier over normal median rate
        threshold = median_vol * burst_multiplier
        burst_windows = time_series[time_series > threshold].index
        
        # If any bursts found, flag all comments in those windows
        if not burst_windows.empty:
            for bw in burst_windows:
                start_time = bw
                end_time = bw + pd.Timedelta(minutes=burst_window_minutes)
