"""Stage 25: Coordinated Inauthentic Behavior (CIB) Detection (s25_cib_detection.py)
...
def run_cib_detection():
    ...
    stage_cfg = config.get("stage_25_cib", {})
    time_window = int(stage_cfg.get("time_window_seconds", 120))
    min_cooccurrences = int(stage_cfg.get("min_cooccurrences", 2))

    print("  -> Analyzing multi-account temporal synchronization...")
    df_rings, df_cib_comments = detect_cib_rings(df, time_window_seconds=time_window, min_cooccurrences=min_cooccurrences)
